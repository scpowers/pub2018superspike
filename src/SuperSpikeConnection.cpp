...
// std::set< SuperSpikeConnection* > SuperSpikeConnection::m_instances = std::set< SuperSpikeConnection* >();

AurynFloat SuperSpikeConnection::beta = 1.0/1e-3; //!< steepness/temperature of nonlinearity

void SuperSpikeConnection::init(AurynFloat eta, AurynFloat feedback_delay, AurynFloat maxweight, AurynState tau_syn, AurynState tau_mem)
{
	...
	logger->debug("SuperSpikeConnection complex matrix finalize");
...
void SuperSpikeConnection::free()
{
	logger->debug("SuperSpikeConnection free");
	// store instance in static set
...
SuperSpikeConnection::SuperSpikeConnection(
		SpikingGroup * source, 
...
AurynWeight SuperSpikeConnection::instantaneous_partial(NeuronID loc_post)
{
...
void SuperSpikeConnection::add_to_syntrace(const AurynLong didx, const AurynDouble input)
{
...
void SuperSpikeConnection::add_to_err(NeuronID spk, AurynState val) 
{
...
void SuperSpikeConnection::compute_van_rossum_err()
{
	// reset
	err->set_zero();
...
/* \brief Computes error signal */
void SuperSpikeConnection::compute_err()
{
 ...
 * */
void SuperSpikeConnection::process_plasticity()
{
	...
	if ( regstrength ) {
		for (NeuronID li = 0; li < dst->get_post_size() ; ++li ) {
			...
			for (const NeuronID * c = bkw->get_row_begin(gi) ; 
				...
				AurynWeight de = ( el_val_flt->get(didx) * e - regstrength * *weight * std::pow(tr_post_hom->get(li),4)); 
				el_sum->add_specific(didx, de);
			}
		}
	} else {
...
void SuperSpikeConnection::propagate_forward()
{
...
void SuperSpikeConnection::back_propagate()
{
	// reset the error signal 
	// Warning! Because this is run in propagate, this soluation is not save for 
	// connection objects
	// TODO find a clean solution for this which can accomodate multiple 
	target_error_vector->set_zero();

	// loop over all postsynaptic cells 
	// if ( approximate ) temp->zero_effective_zeros(gamma);
	for ( NeuronID localid = 0 ; localid < dst->get_post_size() ; ++localid ) {
		const AurynState current_error_value = err->get(localid);
		if ( current_error_value == 0.0 ) continue; 

		NeuronID post_neuron = dst->global2rank(localid); 
		// loop over all presynaptic partners
		for (const NeuronID * c = bkw->get_row_begin(post_neuron) ; c != bkw->get_row_end(post_neuron) ; ++c ) {
			// get transposed weight
			AurynWeight * weight = bkw->get_data(c);
			const AurynState bp_error = *weight * current_error_value; 
			targeted_transmit(src, target_error_vector, *c, bp_error );
		}
	}
}


void SuperSpikeConnection::propagate()
{
	// propagate spikes
	propagate_forward();
	if ( dst->evolve_locally() && back_propagate_error_signals ) back_propagate();
}
...
void SuperSpikeConnection::evolve()
{
...
void SuperSpikeConnection::set_tau_rms( AurynState tau )
{
	tau_rms = tau;
	rms_mul = std::exp(-auryn_timestep*timestep_rmsprop_updates/tau_rms);
}

void SuperSpikeConnection::set_tau_syn( AurynState tau )
{
	tau_syn_ = tau;
	tr_pre->set_timeconstant(tau_syn_); 
}

void SuperSpikeConnection::set_tau_mem( AurynState tau )
{
	tau_mem_ = tau;
	tr_pre_psp->set_timeconstant(tau_mem_); 
}

void SuperSpikeConnection::set_target_group( SpikingGroup * group )
{
	trg = group;
...
double SuperSpikeConnection::get_mean_square_error( )
{
	return scale_tr_err_flt*avgsqrerr->mean()/(1.0-std::exp(-sys->get_time()/tau_avg_err)+1e-9);
}

void SuperSpikeConnection::set_learning_rate( double eta )
{
	eta_   = eta;
}
